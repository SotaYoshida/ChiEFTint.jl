function make_chiEFTint(;is_plot=false)
    ...
    @timeit to "prep." begin
        ...
        ntmp = 3; if v_chi_order > 0;ntmp=n_meshP;end
        xrP_fm,wrP = Gauss_Legendre(0.0,Pmax_fm,ntmp)
        xrP = xrP_fm .* hc   
        RNL = Rnl_all_ab(lcmax,br,ntmp,xrP_fm)       
        ...
        QWs = [ ];wsyms=[]
        if calc_3N
            prep_Fis!(xr,F0s,F1s,F2s,F3s)
            QWs = prep_QWs(xr,ts,ws)
            wsyms = prep_wsyms()
        end
    end

    LECs =zeros(Float64,41) 
    ...
    itnum = 10
    for it = 1:itnum
        ## If you want to optimize (or try samplings)
        ## change itnum, insert a function to
        # update/optimize (MCMC, BayesOpt., etc.) the LECs here
        
        ## and then execute read_LECs!() again.
        ##
        if it > 1;
            for i=1:length(numst2); V12mom[i] .= 0.0;end
        end
        @timeit to "calcV" begin
            ...
            if calc_3N
                calc_vmom_3nf(dLECs,ts,ws,xr,V12mom,dict_numst,to,
        ...
        if is_plot
            @timeit to "momplot" begin
                ...
                #tpath = "../nnint_simple/vmom_1d_N3LO_EM500_bare.txt"
                tpath = "../hartree_fock_NNforce_IMSRG_etc/newNNint/vmom_1d_N3LO_EM500_bare.txt"
                momplot(xr,V12mom,dict_numst[tpnrank],tpnrank,llpSJ_s;ctext=tl,fpath=tpath)
            end
        end
        ...
        @timeit to "Vtrans" begin
            ...
            TMtrans(dLECs,xr,wr,xrP,wrP,Rnl,RNL,nTBME,infos,izs_ab,
