function prep_Fis!(xr,F0s,F1s,F2s,F3s)
...
function calc_vmom_3nf(LECs,ts,ws,xr,V12mom,dict_numst,to,
                       F0s,F1s,F2s,F3s,QWs,wsyms,
                       lsjs,llpSJ_s,tmp_llsj)
    LamChi = 700.0/hc
    ...
    r_c1 = LECs["c1_NNLO"] *1.e-3 *gA^2 * mpi2  / ((Fpi/hc)^4) *hc^2
    r_c3 = LECs["c3_NNLO"] *1.e-3 *gA^2 / (2.0*(Fpi/hc)^4) *hc^2 
    r_c4 = LECs["c4_NNLO"] *1.e-3 *gA^2 / (2.0*(Fpi/hc)^4) *hc^2 
    r_cD = LECs["cD"] * hc * abs(gA) / (8.0*(Fpi/hc)^4 *LamChi)
    ## contact: V_E(CON)
    r_cE= -6.0/4.0 * LECs["cE"] * rho / ( (Fpi/hc)^4 *LamChi) * hc
    V_E(xr,r_cE*bbf,V12mom,dict_numst,to)
...
function V_E(xr,cE,V12mom,dict_numst,to)
    nreg = 3
    pfunc = f1
    for pnrank = 1:3
        l=0;lp=0;S=0;J=0 ##1S0
        calc_Vmom!(pnrank,V12mom,dict_numst[pnrank],xr,cE,cE,
    ...
    @threads for i = 1:n_mesh #for (i,x) in enumerate(xr)
        ...
        for (j,y) in enumerate(xr)
            ...
            for ell = 0:lmax+1
                ...
                QWL0s[ell+1][i,j] = QWL(x_fm,y_fm,ell,ell,0,ts,ws,mpi2)   
            end
            for ell=0:dim
                QWL2s[ell+1][i,j] = QWL(x_fm,y_fm,ell,ell,2,ts,ws,mpi2)
                QWL1s[ell+1][i,j] = QWL(x_fm,y_fm,ell,ell,1,ts,ws,mpi2)
                QWlss[ell+1][i,j] = QWls(x_fm,y_fm,ell,ell,0,ts,ws,mpi2)
                for ellp = 0:dim
                    ndQW1s[ell+1][ellp+1][i,j] = QWL(x_fm,y_fm,ell,ellp,1,ts,ws,mpi2;is_xmul=false)
                    ndQW2s[ell+1][ellp+1][i,j] = QWL(x_fm,y_fm,ell,ellp,2,ts,ws,mpi2;is_xmul=false)
                end 
            end
        end
    end
...
function QWL(k,kp,ell,ellp,p,ts,ws,mpi2;is_xmul=true)
    dtdk3 = 0.5 *kF
    s = 0.0
...
function QWls(k,kp,ell,ellp,p,ts,ws,mpi2)
    dtdk3 = 0.5 *kF
    s = 0.0
    deno = 2.0 * (2*pi)^2
    if p  == 0 
